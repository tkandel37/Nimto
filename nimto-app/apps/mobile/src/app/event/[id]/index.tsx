...
export default function EventScreen() {
  ...
  const duplicate = useMutation({
    mutationFn: () => apiRequest<UserEvent>(`/events/${id}/duplicate`, { method: "POST", token }),
  ...
  const toggleArchive = useMutation({
    mutationFn: (action: "archive" | "restore") => apiRequest<UserEvent>(`/events/${id}/${action}`, { method: "POST", token }),
  ...
  const actionError = duplicate.error || toggleArchive.error || remove.error || updatePublish.error;
  ...
  return <Screen>
    ...
    <Card><View style={uiStyles.between}><View style={styles.statusCopy}><Text style={uiStyles.sectionTitle}>Invitation status</Text><Text style={uiStyles.muted}>{event.archivedAt ? "Archived events have disabled public links." : event.isPublished ? "Your public invitation link is live." : "Publish when the invitation is ready."}</Text></View><Text style={[styles.status, event.isPublished && !event.archivedAt && styles.statusLive]}>{event.archivedAt ? "Archived" : event.isPublished ? "Live" : "Draft"}</Text></View><Button busy={updatePublish.isPending} disabled={Boolean(event.archivedAt)} onPress={() => updatePublish.mutate(!event.isPublished)} title={event.isPublished ? "Unpublish" : "Publish invitation"} variant={event.isPublished ? "secondary" : "primary"} /></Card>
    ...
    <View style={styles.actions}>
      ...
      <Action title="Duplicate event" detail="Create a new unpublished copy with the same design" onPress={() => duplicate.mutate()} />
    </View>
    <Button disabled={!event.isPublished || Boolean(event.archivedAt)} onPress={shareEvent} title="Share public invitation" />
    {!event.isPublished || event.archivedAt ? <Text style={styles.hint}>{event.archivedAt ? "Restore the event before sharing its link." : "Publish the event before sharing its public link."}</Text> : null}
    <Card>
      <Text style={uiStyles.sectionTitle}>Event lifecycle</Text>
      <Text style={uiStyles.muted}>Archive an event to disable its public links without deleting its history.</Text>
      <Button
        busy={toggleArchive.isPending}
        onPress={() => {
          if (event.archivedAt) return toggleArchive.mutate("restore");
          Alert.alert("Archive event?", "This will disable the event's public and guest links until it is restored.", [
            { text: "Cancel", style: "cancel" },
            { text: "Archive", onPress: () => toggleArchive.mutate("archive") },
          ]);
        }}
      ...
      <Button
        ...
        onPress={() => Alert.alert("Permanently delete event?", `Delete “${event.title}” and all of its guest links? This cannot be undone.`, [
          { text: "Cancel", style: "cancel" },
          { text: "Delete permanently", style: "destructive", onPress: () => remove.mutate() },
        ])}
        title="Delete event permanently"
        variant="danger"
      />
    </Card>
    {actionError ? <Text style={styles.error}>{actionError instanceof Error ? actionError.message : "The event action could not be completed."}</Text> : null}
  </Screen>;
}
...
const styles = StyleSheet.create({ stats: { flexDirection: "row", gap: spacing.sm }, stat: { flex: 1, padding: 12 }, statusCopy: { flex: 1 }, status: { color: colors.warning, fontWeight: "800" }, statusLive: { color: colors.success }, actions: { gap: spacing.sm }, actionTitle: { color: colors.ink, fontSize: 16, fontWeight: "800" }, arrow: { color: colors.plum, fontSize: 30 }, hint: { color: colors.muted, fontSize: 12, textAlign: "center" }, error: { color: colors.danger, fontSize: 13 } });
