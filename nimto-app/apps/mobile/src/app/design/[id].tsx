...
export default function DesignPreviewScreen() {
      ...
      <View style={styles.center}>
        <Text style={styles.error}>
          {query.error instanceof Error
            ? query.error.message
            : "Could not open this design."}
        </Text>
        <Button onPress={() => query.refetch()} title="Try again" />
        <Button
          onPress={() => router.back()}
  ...
  if (!design || !version)
    return (
      <View style={styles.center}>
        <Text style={uiStyles.sectionTitle}>Design unavailable</Text>
        <Text style={uiStyles.muted}>
          This design may have been unpublished. Choose another from the
          catalogue.
        </Text>
        <Button
  ...
  return (
    <Screen>
      <PageHeader
        eyebrow={
          [design.category?.name, design.subcategory?.name]
            .filter(Boolean)
            .join(" · ") || "Invitation design"
        }
        title={design.name}
        detail="Preview the complete invitation before deciding. You only need an account when you choose to use it."
      />
      <Card style={styles.previewCard}>
        <View style={styles.preview}>
          <InvitationPreview
            html={version.rawHtml}
      ...
      <Button
        onPress={useDesign}
        title={token ? "Use this design" : "Use this design · Sign in"}
      />
      <Button
        onPress={() => router.back()}
        title="Keep browsing"
      ...
      {!token ? (
        <Text style={styles.note}>
          Browsing is free. Signing in saves this choice and lets you create the
          invitation.
        </Text>
      ) : null}
    </Screen>
