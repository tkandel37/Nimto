...
export default function DesignsScreen() {
  ...
  const visible = useMemo(
    () =>
      (designs.data ?? []).filter((design) => {
        const matchesCategory = !category || design.category?.id === category;
        const value = search.trim().toLowerCase();
        return (
          matchesCategory &&
          (!value ||
            [design.name, design.category?.name, design.subcategory?.name]
              .filter(Boolean)
  ...
  return (
    <Screen>
      <PageHeader
        eyebrow="Invitation catalogue"
        title="Choose your design"
        detail={
          token
            ? "Start with a published design, then personalize every editable field."
            : "Browse and preview freely. Sign in only when you are ready to use a design."
        }
      />
      <TextInput
      ...
      <View style={styles.chips}>
        <Pressable onPress={() => setCategory("")}>
          <Text style={[styles.chip, !category && styles.chipActive]}>All</Text>
        </Pressable>
      ...
      {designs.isError ? (
        <EmptyState
          action={
            <Button onPress={() => designs.refetch()} title="Try again" />
          }
          detail={
            designs.error instanceof Error
              ? designs.error.message
              : "Could not load designs."
          }
          title="Could not load designs"
        />
      ) : null}
      {!designs.isLoading && !designs.isError && !visible.length ? (
        <EmptyState
          detail="Try a broader search or a different category."
      ...
      <View style={styles.grid}>
        {visible.map((design) => {
          ...
          const previewHtml = version?.thumbnailHtml || version?.rawHtml;
          return (
            <Pressable
              key={design.id}
              onPress={() => version && router.push(`/design/${design.id}`)}
            ...
            >
              <Card style={styles.designCard}>
                ...
                <Text numberOfLines={1} style={styles.designName}>
                  {design.name}
                </Text>
                <Text style={uiStyles.muted}>
                  {design.category?.name ?? "Invitation"} · Preview
                </Text>
              </Card>
            </Pressable>
          );
